...
ID =123;
grname=['../Data/PointCloud/' num2str(ID) '.mat'];%把数值转换成字符串， 转换后可以使用fprintf或disp函数进行输出
load(grname)
Ground(:,1) = -Ground(:,1);
Ground(:,2) = -Ground(:,2);
...
idx_g = (Ground(:,1:2)*a11(1:2) > -a11(3)+0.5*norm(a11(1:2))) | ...
    (Ground(:,1:2)*a21(1:2) > -a21(3)+0.5*norm(a11(1:2)));
pts_g =  Ground(idx_g,1:3);
dist = abs(a11(1:2)'*Ground(:,1:2)'-1)/norm(a11(1:2));
idx1 = dist'<0.5;
dist = abs(a21(1:2)'*Ground(:,1:2)'-1)/norm(a21(1:2));
idx2 = dist'<0.5;
idx_b = idx1|idx2;
